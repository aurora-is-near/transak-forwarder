...
const readFromBodyOrQueryParams = (
  ...
  const value =
    (body && body[parameterName]) || searchParams.get(parameterName);
  if (!value) {
    return null;
  }
  return value;
...
async function handleRequest(req: NextRequest) {
  ...
  const walletAddress = await readFromBodyOrQueryParams(
    req,
    body,
    "walletAddress"
  );
  const siloEngineAccountId = await readFromBodyOrQueryParams(
    req,
    body,
    "silo"
  );
  ...
  const fiatAmount =
    (await readFromBodyOrQueryParams(req, body, "fiatAmount")) || "50";
  const fiatCurrency =
    (await readFromBodyOrQueryParams(req, body, "fiatCurrency")) || "EUR";
  const defaultFiatAmount =
    (await readFromBodyOrQueryParams(req, body, "defaultFiatAmount")) || "0";
  const defaultCryptoAmount =
    (await readFromBodyOrQueryParams(req, body, "defaultCryptoAmount")) || "0";
  const cryptoCurrencyCode =
    (await readFromBodyOrQueryParams(req, body, "cryptoCurrencyCode")) ||
    "NEAR";
  const environment =
    (await readFromBodyOrQueryParams(req, body, "environment")) || "staging";
  ...
  const forwarderAddress = await getForwarderAddress(
    targetAddress,
    siloEngineAccountId
  );
  ...
  if (!widgetUrl) {
    return new NextResponse("Failed to create Transak session", {
      status: 500,
    });
  }
